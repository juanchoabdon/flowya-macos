...
export function SparkleIcon({ size = 16 }: { size?: number }) {
  const gradId = `aiWaveGrad_${size}_${Math.random().toString(36).slice(2, 6)}`;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className="ai-wave-svg">
      <defs>
      ...
      <path d="M2 8c2-2 4-2 6 0s4 2 6 0 4-2 6 0" stroke={`url(#${gradId})`} strokeWidth="2" strokeLinecap="round" fill="none"/>
      <path d="M2 13c2-2 4-2 6 0s4 2 6 0 4-2 6 0" stroke={`url(#${gradId})`} strokeWidth="2" strokeLinecap="round" fill="none"/>
      <path d="M2 18c2-2 4-2 6 0s4 2 6 0 4-2 6 0" stroke={`url(#${gradId})`} strokeWidth="2" strokeLinecap="round" fill="none"/>
    </svg>
  );
}
